...
- [`--environment (-e) <NEW_ENVIRONMENT>`](#arg---environment) : A name for the created environment. Without `--feature` the imported content is written inline on the environment

- [`--feature (-f) <NEW_FEATURE>`](#arg---feature) : A name for the created feature. The feature is added to the environment of the same name unless `--environment` is given
