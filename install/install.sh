...
IS_MSYS=false
[ "$(uname -o)" != "Msys" ] || IS_MSYS=true

case "${PLATFORM-}" in
    Darwin) PLATFORM="apple-darwin" ;;
    Linux) PLATFORM="unknown-linux-musl" ;;
    *) ! $IS_MSYS || PLATFORM="pc-windows-msvc" ;;
esac
...
if $HAVE_CURL; then
  ...
  case "$CURL_ERR" in
      35|53|54|59|66|77)
         ...
         # fallback to wget
         ;;
      0) if [ "${HTTP_CODE}" -lt 200 ] || [ "${HTTP_CODE}" -gt 299 ]; then
...
if [ -n "${PIXI_NO_PATH_UPDATE:-}" ]; then
  ...
  case "$(basename "$SHELL")" in
      ...
      *)
          ...
          echo "warn: Please permanently add '${BIN_DIR}' to your \$PATH to enable the 'pixi' command." >&2
          ;;
  esac
fi
}; __wrap__
