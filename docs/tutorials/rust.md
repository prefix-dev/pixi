...
`pixi run` is pixi's way to run commands in an installation environment. It will make sure that the installation environment is activated for the command to run.
...
You can also activate the installation environment in a shell by running `pixi shell`, after that you don't need `pixi run ...` anymore.
