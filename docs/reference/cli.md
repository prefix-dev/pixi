...
- `--pyproject-toml`: Create a `pyproject.toml` manifest, rather than a `pixi.toml` manifest. Recommended for a python project.
- `--pixi-toml`: Create a `pixi.toml` manifest, rather than a `pyproject.toml` manifest. Use when there is already a `pyproject.toml` in the directory.
...
pixi init --pyproject-toml myproject
pixi init --pixi-toml myproject # Use when there is already a pyproject.toml in the directory.
```
