...
pub fn default_bytes_style() -> indicatif::ProgressStyle {
    indicatif::ProgressStyle::default_bar()
        .template("    {prefix:20!} [{elapsed_precise}] [{wide_bar:.bright.yellow/dim.white}] {bytes:>8} @ {smoothed_bytes_per_sec:8}").unwrap()
        .progress_chars("━━╾─")
        .with_key(
...
pub fn default_progress_style() -> indicatif::ProgressStyle {
    indicatif::ProgressStyle::default_bar()
        .template("    {prefix:20!} [{elapsed_precise}] [{wide_bar:.bright.yellow/dim.white}] {pos:>7}/{len:7}").unwrap()
        .progress_chars("━━╾─")
}
