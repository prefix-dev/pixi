...
mod tests {
    ...
    impl InstalledDistBuilder {
        pub fn registry<S: AsRef<str>>(name: S, version: S, path: PathBuf) -> InstalledDist {
            ...
            let version = uv_pep440::Version::from_str(version.as_ref())
                .expect("cannot parse pep440 version");
    ...
    impl InstalledDistOptions {
        ...
        fn with_requires_python<S: AsRef<str>>(mut self, requires_python: S) -> Self {
            self.requires_python = uv_pep440::VersionSpecifiers::from_str(requires_python.as_ref())
                .ok()
                .map(|v| v);
            self
        }
    ...
    impl MockedSitePackages {
        ...
        fn create_file_backing(
            ...
            std::fs::write(dist_info.join("INSTALLER"), installer)
                .expect("could not write INSTALLER");
    ...
    impl PyPIPackageDataBuilder {
        ...
        fn directory<S: AsRef<str>>(
            ...
            rattler_lock::PypiPackageData {
                ...
                requires_dist: vec![],
                requires_python: None,
                location: UrlOrPath::Path(Utf8TypedPathBuf::from(
    ...
    impl RequiredPackages {
        ...
        fn add_registry<S: AsRef<str>>(mut self, name: S, version: S) -> Self {
            let package_name = uv_normalize::PackageName::new(name.as_ref().to_owned())
