...
use pixi_manifest::{DependencyOverwriteBehavior, FeatureName, PrioritizedChannel, SpecType};
...
async fn import_conda_env(args: Args) -> miette::Result<()> {
    ...
    // add environment matching feature name if one does not already exist
    if workspace
        .workspace()
