...
1. A `LinkingTo` field in the `DESCRIPTION` file
...
1. **Compiler Detection**: Auto-detects or uses configured compilers based on package structure
1. **Recipe Generation**: Creates a conda recipe with all dependencies converted to conda format
1. **Build Script**: Generates a platform-appropriate script that:
...
1. **Package Creation**: Creates a platform-specific conda package
...
- [Build Backends Overview](../) - Overview of all available build backends
