...
def test_run_succeeds_from_a_warm_cache(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, dummy_http: str
) -> None:
    """The success half of the test above: with the packages cached, the
    restricted solve resolves them and the task runs without network."""
    env = isolated_cache(tmp_path)
    warm_package_cache(pixi, dummy_http, tmp_path / "warmup", env)

    manifest = write_manifest(tmp_pixi_workspace, dummy_http)
    manifest.write_text(manifest.read_text() + '\n[tasks]\nhello = "echo offline-hi"\n')

    verify_cli_command(
        [pixi, "run", "--manifest-path", manifest, "--offline", "hello"],
        env=env,
        stdout_contains="offline-hi",
    )


def test_offline_env_var_restricts_the_solve(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, dummy_http: str
) -> None:
    """`PIXI_OFFLINE` must route into the restriction exactly like the flag."""
    manifest = write_manifest(tmp_pixi_workspace, dummy_http)
    env = isolated_cache(tmp_path)
    warm_repodata(pixi, manifest, env)
...
def test_add_failure_leaves_the_manifest_untouched(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, versions_http: str
) -> None:
    """When the restricted solve fails, `pixi add` must not record the new
    dependency."""
    env = isolated_cache(tmp_path)
    warm = tmp_path / "warm"
    warm.mkdir()
    warm_repodata(pixi, write_manifest(warm, versions_http, dependency='package = "*"'), env)
...
def test_upgrade_failure_leaves_the_manifest_untouched(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, dummy_http: str
) -> None:
    env = isolated_cache(tmp_path)
    manifest = write_manifest(tmp_pixi_workspace, dummy_http)
