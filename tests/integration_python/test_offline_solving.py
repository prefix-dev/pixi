...
def test_remote_records_are_excluded_with_a_reason(
    ...
    # The failure is caused by offline mode, so the hint for getting out of
    # it must be attached.
    assert "remove the `--offline` flag" in output.stderr
...
def test_multi_platform_manifest_relocks_offline(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, http_channel: str
) -> None:
    """The restriction only covers the solve for the platform pixi runs on.
    Packages for the other platforms in the manifest never enter this
    machine's package cache, so restricting their solves would make every
    multi-platform lock-file rewrite fail offline; they solve from cached
    repodata instead."""
    env = cache_env(tmp_path / "cache")
    other = "osx-arm64" if CURRENT_PLATFORM != "osx-arm64" else "linux-64"
    manifest = write_manifest(
        tmp_pixi_workspace,
        http_channel,
        platforms=f'["{CURRENT_PLATFORM}", "{other}"]',
    )
    ...
    assert f"{other}: " in lock_file or f"{other}:" in lock_file


def test_install_offline_without_a_lock_file_from_a_warm_cache(
    pixi: Path, tmp_pixi_workspace: Path, tmp_path: Path, http_channel: str
) -> None:
    """The full offline pipeline in one command: with everything cached but no
    lock file, `pixi install --offline` has to run a restricted solve and then
    install from the cache alone."""
    env = cache_env(tmp_path / "cache")
    manifest = write_manifest(tmp_pixi_workspace, http_channel)
    verify_cli_command([pixi, "install", "--manifest-path", manifest], env=env)
