...
use crate::common::{PixiControl, package_database::{Package, PackageDatabase}};
...
async fn test_build_with_relative_source_path() {
    ...
    let alternative_source_dir = pixi.workspace_path().parent().unwrap().join("alternative-source");
...
async fn test_build_with_absolute_source_path() {
    ...
    fs::write(absolute_source_dir.join("recipe.yaml"), "schema_version: 1\n").unwrap();
