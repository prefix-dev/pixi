...
Pixi and RoboStack are currently a Tier 3 platform in the ROS ecosystem, which means they are community-supported but not officially supported by Open Robotics. [Open Robotics has communicated](https://www.openrobotics.org/technology-strategy-2026) that Pixi will be part of their strategy to expand accessibility and ease of use. So with your help, we can grow the community and make it a first-class way to use ROS on any platform.
...
- **[Reproducible environments](../workspace/lock_file/)**\
...
- **[Works on Linux, macOS, and Windows](../workspace/multi_platform_configuration/)**\
...
- **[Built-in task runner](../workspace/advanced_tasks/)**\
...
- **[Package and distribute your own nodes](../build/ros/)**\
  Build your ROS packages as conda packages with the [`pixi-build-ros`](../build/backends/pixi-build-ros/) backend and publish them to a private channel.
...
| Guide                                                       | Description                                                           |
| ----------------------------------------------------------- | --------------------------------------------------------------------- |
| [ROS 2 Tutorial](../tutorials/ros2/)                        | Set up a workspace, write Python and C++ nodes, use `pixi run` tasks. |
| [Building a ROS Package](../build/ros/)                     | Package your ROS nodes as conda packages based on your `package.xml`  |
| [pixi-build-ros Backend](../build/backends/pixi-build-ros/) | Backend reference for building ROS packages.                          |
| [GitHub Actions](../integration/ci/github_actions/)         | Run your ROS tests in CI.                                             |
| [Distributing with Pixi Pack](../deployment/pixi_pack/)     | Bundle a workspace for offline or embedded deployment.                |
...
Browse available packages for each distribution on [prefix.dev](https://prefix.dev/channels/). Package names follow the pattern `ros-<distro>-<package-name>`, mirroring the apt naming convention. There are more packages on the `prefix.dev` channels so some might normally come from `apt` or `pip` instead, but they are all available in a single place for Pixi:
...
- Package it yourself with Pixi using the [`pixi-build-ros` backend](../build/backends/pixi-build-ros/) and publish it to your own conda channel.
...
[`pixi-ros`](https://github.com/ruben-arts/pixi-ros) can be used to quickstart a ROS workspace. This project can initialize a workspace with the correct RoboStack channels and ROS dependencies, and generate `pixi.toml` files for ROS packages.
...
See [`pixi workspace platform`](../reference/cli/pixi/workspace/platform/) for the full list of friendly virtual-package keys (`cuda`, `glibc`, `macos`, …) and for editing or removing entries.
