...
- **Type**: `Boolean`
- **Default**: `false`
- **Target Merge Behavior**: `Or` — if either the base or platform-specific value is `true`, discovery is skipped
...
1. **Dependency Resolution**: Maps ROS dependencies to conda packages using RoboStack mappings
1. **Environment Setup**: Configures ROS-specific environment variables
1. **Build Execution**: Uses the appropriate build template based on package type.
1. **Installation**: Installs built artifacts to the conda package prefix
