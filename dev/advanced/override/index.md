...
Overrides can also change where a transitive package comes from and which `exclude-newer` cutoff is used for that package.
...
This is useful when most of the environment should stay pinned to an older cutoff, but a specific package needs a newer release stream from a separate index.
...
If the same package is present in both places, the dependency override's `exclude-newer` value is used for that package during PyPI resolution.
