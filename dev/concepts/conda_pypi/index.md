...
- set `exclude-newer` in [`pypi-dependencies`](../../reference/pixi_manifest/#pypi-dependencies) for a direct package
- set it in `pypi-options.dependency-overrides` for a transitive package

PyPI does not have a conda-style per-channel cutoff. When you use a separate package index, pin the package with `index = "..."` and set `exclude-newer` on that package.
