...
use rattler_lock::{
    LockedPackageRef, PackageHashes, PypiIndexes, PypiSourceTreeHashable, UrlOrPath,
};
use thiserror::Error;
                                            ...
                                            .map_err(|e| {
                                                CommandDispatcherError::Failed(Box::new(
                                                    PlatformUnsat::FailedToReadLocalMetadata(
                                                        pkg.name.clone(),
                                                        format!(
                                                            "failed to initialize UV context: {e}"
                                                        ),
                                                    ),
                                                ))
                                            })?;
...
fn compare_source_tree_hash(
    directory: &Path,
    locked_record: &UnresolvedPypiRecord,
) -> Option<PlatformUnsat> {
    let locked_data = locked_record.as_package_data();
