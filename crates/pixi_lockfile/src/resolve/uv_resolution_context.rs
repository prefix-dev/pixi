...
impl UvResolutionContext {
    pub fn from_workspace_config(project_config: &Config) -> miette::Result<Self> {
        ...
        let keyring_provider = match project_config.pypi_config().use_keyring() {
        ...
        let allow_insecure_host = project_config
            .pypi_config
            .allow_insecure_host
            .iter()
            .try_fold(
        ...
        Ok(Self {
            ...
            extra_middleware: ExtraMiddleware(uv_middlewares(project_config)),
            proxies: project_config.get_proxies().into_diagnostic()?,
