...
pub struct Args {
    ...
    /// Add the requirements as PEP 723 PyPI dependencies.
    #[arg(long)]
    pub pypi: bool,
...
mod tests {
    ...
    #[test]
    fn accepts_conda_and_explicit_pypi_dependencies() {
        let args = Args::try_parse_from(["add", "example.py", "python"]).unwrap();
        assert!(!args.pypi);
        assert_eq!(args.specs, ["python"]);

        let args = Args::try_parse_from(["add", "--pypi", "example.py", "requests>=2"]).unwrap();
        assert!(args.pypi);
        assert_eq!(args.specs, ["requests>=2"]);
    }
