...
pub struct Args {
    ...
    ///   /path/to/local/channel
    ///
    /// A local channel is initialized and indexed automatically. To copy the
    /// built package(s) into a directory without creating a channel, use
    /// `--target-dir` instead.
