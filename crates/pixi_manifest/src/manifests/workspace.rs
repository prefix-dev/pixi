...
"#;
    ...
    #[test]
    fn test_exclude_newer_config_without_workspace_cutoff() {
        struct TestFeatures<'a> {
            manifest: &'a WorkspaceManifest,
            features: Vec<&'a Feature>,
        }
        ...
        // No workspace-level `exclude-newer`, so the resolved default cutoff is
        // the far-future sentinel that used to overflow the jiff conversion.
        let contents = r#"
[project]
name = "foo"
channels = ["conda-forge", { channel = "bioconda", exclude-newer = "2015-12-02T02:07:43Z" }]
platforms = []

[exclude-newer]
polars = "2016-12-02T02:07:43Z"
"#;
        ...
        assert_eq!(
            config.cutoff_for_package(&PackageName::from_str("polars").unwrap(), None),
            "2016-12-02T02:07:43Z".parse::<jiff::Timestamp>().unwrap()
        );
        assert_eq!(
            config.cutoff_for_package(
                &PackageName::from_str("numpy").unwrap(),
                Some(bioconda.as_str())
            ),
            "2015-12-02T02:07:43Z".parse::<jiff::Timestamp>().unwrap()
        );
        assert_eq!(
            ...
            jiff::Timestamp::MAX,
            "packages outside an override must not be excluded"
        );
    }
