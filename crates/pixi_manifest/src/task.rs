...
impl Task {
    ...
    pub fn inputs(&self) -> Option<&GlobPatterns> {
        match self {
            ...
            Task::Alias(alias) => alias.inputs.as_ref(), // Add this
            _ => None,
        }
    }
    ...
    pub fn outputs(&self) -> Option<&GlobPatterns> {
        match self {
            ...
            Task::Alias(alias) => alias.outputs.as_ref(), // Add this
            _ => None,
        }
    }
...
pub struct Alias {
    ...
    /// A list of glob patterns that should be watched for changes
    pub inputs: Option<GlobPatterns>,

    /// A list of glob patterns that are generated by this task
    pub outputs: Option<GlobPatterns>,
}
        ...
        match value {
                ...
                if !process.depends_on.is_empty() {
                    ...
                    for dep in &process.depends_on {
                        if dep.args.is_some() || dep.environment.is_some() {
                            ...
                            deps.push(Value::from(dep.task_name.to_string()));
                        }
                    }
                    table.insert("depends-on", Value::Array(deps));
                }
            ...
            Task::Alias(alias) => {
                ...
                if let Some(args_vec) = &alias.args {
                    ...
                    table.insert("args", Value::Array(args));
                }

                // 2. Inputs/Outputs
                if let Some(inputs) = &alias.inputs {
                    table.insert(
                        "inputs",
                        Value::Array(Array::from_iter(inputs.iter().map(|i| i.source()))),
                    );
                }
                if let Some(outputs) = &alias.outputs {
                    table.insert(
                        "outputs",
                        Value::Array(Array::from_iter(outputs.iter().map(|o| o.source()))),
                    );
                }

                // 3. Depends On
                let mut deps_array = Array::new();
                for dep in &alias.depends_on {
                    if dep.args.is_some() || dep.environment.is_some() {
                        let mut dep_table = Table::new().into_inline_table();
                        dep_table.insert("task", dep.task_name.to_string().into());
                        if let Some(args) = &dep.args {
                            dep_table.insert(
                                ...
                                Value::Array(Array::from_iter(args.iter().map(|arg| match arg {
                                    ...
                                    DependencyArg::Named(name, val) => {
                                        let mut t = InlineTable::new();
                                        t.insert(name, Value::from(val.source().to_string()));
                                        Value::InlineTable(t)
                                    }
                                }))),
                            );
                        ...
                        deps_array.push(Value::InlineTable(dep_table));
                    } else {
                        deps_array.push(Value::from(dep.task_name.to_string()));
                    }
                }
                table.insert("depends-on", Value::Array(deps_array));
                ...
                // Logic: If it's JUST a list of dependencies with no other fields, return the array directly.
                // Otherwise, return the full inline table.
                if alias.args.is_none()
                    && alias.description.is_none()
                    && alias.inputs.is_none()
                    && alias.outputs.is_none()
                {
                    Item::Value(
                        table
                            .remove("depends-on")
            ...
            Task::Custom(custom) => {
                ...
                // Custom tasks often represent CLI calls, so we include the templated flag
                table.insert("templated", custom.templated.into());
                Item::Value(Value::InlineTable(table))
            }
        }
    }
...
mod tests {
    ...
    fn test_template_rendering_basic() {
        ...
        let ts = TemplateString::from("Running on {{ pixi.platform }}");
        assert_eq!(ts.render(&context).unwrap(), "Running on linux-64");

        // Test boolean flags
        let ts_win = TemplateString::from("{% if pixi.is_win %}cmd.exe{% else %}bash{% endif %}");
        assert_eq!(ts_win.render(&context).unwrap(), "bash");
    }

    #[test]
    fn test_template_rendering_with_args() {
        let typed_args = ArgValues::TypedArgs(vec![TypedArg {
        ...
        let ts = TemplateString::from("install --version {{ version }}");
        assert_eq!(ts.render(&context).unwrap(), "install --version 1.2.3");
    }

    #[test]
    fn test_render_error_reporting() {
        let context = TaskRenderContext::default();

        // This should fail because of the malformed syntax {{ ... %}
        let ts = TemplateString::from("{{ invalid syntax %}");
        let result = ts.render(&context);

        assert!(result.is_err());

        if let Err(e) = result {
            assert!(e.get_source().contains("invalid syntax"));
        }
    }

    #[test]
    fn test_complex_pixi_object() {
        // Using Name variant if it exists, otherwise Default
        let env_name = EnvironmentName::Default;
        let context = TaskRenderContext {
            environment_name: &env_name,
            ..Default::default()
        };

        let ts = TemplateString::from("Env is {{ pixi.environment.name }}");
        assert_eq!(ts.render(&context).unwrap(), "Env is default");
    }

    #[test]
    fn test_cmd_args_quoting() {
        let context = TaskRenderContext::default();
        let cmd = CmdArgs::Multiple(vec![
            TemplateString::from("echo"),
            TemplateString::from("hello world"), // Needs quoting
        ]);

        let rendered = cmd.as_single(&context).unwrap().unwrap();
