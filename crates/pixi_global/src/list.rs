...
/// versions. Environments that hold a single dependency named after themselves
/// carry it in the header instead.
fn dependencies_row(
...
pub async fn list_specific_global_environment(
    ...
    // names, counted rather than listed: the table below has the detail. Only
    // the count, since a size here would be read as the size of the
    // environment while leaving out the package that was asked for, which is
    // usually the largest one. The `size` row below carries that instead.
