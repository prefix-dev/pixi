...
impl StateChanges {
    ...
    ///
    /// Rendering the report is not part of the operation, so a failure here
    /// is a warning rather than something that changes the exit code or stops
    /// the environments after this one from being processed.
...
/// environment itself, the case where the header can carry the version without
/// repeating the package name in a row.
...
/// could belong to this package are parsed. Reading the whole prefix would mean
/// parsing every record of every environment on a sync that changes nothing.
...
async fn build_report(
    ...
    // `--force-reinstall` unexposes and re-exposes the same executable, and
    // what the user cares about is that it ended up exposed. Changes are
    // walked in the order they were recorded, so the last one wins.
    ...
    for change in changes {
        match change {
            ...
            // Not reported: the `exposed` row says what is exposed, and a
            // rewritten trampoline does not change that. It happens as a side
            // effect of almost any change to the environment, so reporting it
            // means removing one package talks about another.
    ...
    // An environment that holds a single dependency named after itself already
    // says everything about it in the header, so a row would repeat the name
    // standing right above it. `pixi global list` leaves it out for the same
    // reason.
    ...
    let version = if status == EnvStatus::Removed || !single_package {
        ...
        // reads `0.26.1 -> 0.27.0` in the header the way the row it replaces
        // would have.
