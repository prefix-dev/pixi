...
//! ```
//!
//! The rows are drawn as children of their own header, so a block is
//! self-contained: it never has to know which environment comes after it, and
//! can be printed the moment that environment is done.
//!
//! The labels are the keys of `pixi-global.toml`, so the output doubles as a
//! hint about which field to edit. `transitive`, `completions` and `size`
//! describe the result rather than the manifest and have no key. There is no
//! `dependencies` row above, because the header already names the only
//! dependency this environment has.
...
/// Width of the status column, sized to the longest status. The status leads
/// the header and the name is padded out past it, so both line up down a run
/// without anything having to know how wide the widest header will turn out to
/// be. Parenthesised so it reads as an annotation on the environment rather
/// than as a verb with the name as its object, and left-aligned so a header
/// never sits to the right of the rows hanging off it.
...
impl EnvStatus {
    ...
    /// describes: green adds, yellow changes, red takes away. A status that
    /// used green for "something happened" would collide with the green of an
    /// added item in the rows right underneath it.
    fn style(self) -> Style {
        match self {
...
///
/// These are ASCII on purpose: they are the only signal left when colors are
...
pub struct EnvReport {
    ...
    /// environment, where repeating the name in a row would add nothing.
    ...
    /// of the closing section, never on the marker printed where the failure
    /// happened.
...
impl EnvReport {
    ...
    /// Whether this report is about an environment that failed, either as the
    /// marker where it happened or as its reason in the closing section.
...
impl RenderOptions {
    /// The options matching the terminal this process writes its reports to.
    ///
    /// Reports go to stderr, so that is the stream that decides both the width
    /// and whether colors are used.
...
/// Render the line that accounts for the environments that were considered but
/// left out of the report because nothing changed.
///
/// `secondary` dims it, which is right when it trails the blocks it belongs to
/// and wrong when it is the whole answer.
...
/// Render the heading that opens the closing section, where the reasons of the
/// environments that failed are collected.
///
/// Without it the first reason is a bare environment name at column 0, which is
/// the shape `pixi global list` uses for an environment: nothing would say that
/// a new section had started, and over a run of many environments the reasons
/// sit a screen of scrollback away from the markers they belong to.
...
fn render_item(item: &Item, options: &RenderOptions) -> String {
    ...
    // what the item is. Otherwise a removed package would be drawn in the green
    // that means "added" everywhere else, right next to its own red `-`.
...
///
/// Failures are printed even when quiet, since they are the reason the command
/// exits non-zero. That covers the reason as well as the marker: the reason is
/// no longer a `tracing::warn!`, so filtering it here would leave a quiet run
/// with a name and no diagnosis at all.
...
/// Say that a command found nothing to do, for the case where there was not
/// even an unchanged environment to count. Self-guarding: it stays quiet if
/// anything has already been printed.
...
mod tests {
    ...
    /// A removal is drawn in one color throughout, so nothing about it is
    /// green: that is the color of an addition in the block right above it.
    #[test]
    ...
    fn colors_a_state_listing_by_what_the_items_are() {
        let report = EnvReport::new("dev", None, None).with_rows(vec![
            Row::new(
                Label::Dependencies,
                vec![Item::package(Marker::None, "bat", Some("0.26.1".to_string()))],
            ),
            Row::new(
                Label::Exposed,
    ...
    /// A quiet run still gets the reason: it is no longer a `tracing::warn!`,
    /// so filtering it here would leave a failing name with no diagnosis.
    #[test]
