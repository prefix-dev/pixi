...
impl Config {
    ...
    pub fn offline(&self) -> bool {
        self.extensions.offline.unwrap_or(false)
    }
...
UNUSED = "unused"
    ...
    fn test_config_from_cli() {
        ...
        assert_eq!(config.tls_no_verify, Some(true));
        assert_eq!(config.tls_root_certs, Some(TlsRootCerts::System));
        assert_eq!(config.extensions.offline, Some(true));
        assert_eq!(
        ...
        assert_eq!(config.extensions.offline, None);
        assert_eq!(
    ...
    fn test_offline_config() {
        ...
        // it keeps it enabled.
        let mut config = Config::default();
        config.extensions.offline = Some(true);
        let merged = config.merge_config(Config::default());
        assert_eq!(merged.extensions.offline, Some(true));

        // An explicit `--offline=false` from a higher layer (CLI/env)
        // overrides `offline = true` from a config file.
        let mut config = Config::default();
        config.extensions.offline = Some(true);
        let merged = config.merge_config(Config::from(ConfigCli {
            offline: Some(false),
            ..Default::default()
        }));
        assert_eq!(merged.extensions.offline, Some(false));
