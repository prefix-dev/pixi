...
use crate::injected_config::{BackendOverrideKey, ChannelConfigKey, EnabledProtocolsKey};
...
use pixi_build_discovery::{BackendSpec, CommandSpec, DiscoveryError, SystemCommandSpec};
...
impl BuildBackendMetadataInner {
    ...
    async fn resolve_checkouts(
        ...
        // carried on this request, build the backend from it directly and skip
        // on-disk discovery. The synthetic provenance is anchored at the
        // checked-out source so the backend builds the code that was checked
        // out; the inline manifest carries no `build.source` of its own.
        let discovered_backend = match self.inline.as_ref() {
            Some(inline) => {
                ...
                Arc::new(
                    inline
                        ...
                        .map_err(|err| {
                            BuildBackendMetadataError::Discovery(Arc::new(
