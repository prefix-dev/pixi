...
//! identity: it is a deterministic hash of `(dependency name, inline package
//! table)` computed at manifest-assembly time, so it distinguishes two inline
//! definitions that resolve to the same source location and changes whenever
//! the table is edited.
...
use pixi_build_discovery::DiscoveredBackend;
use pixi_manifest::{
...
impl InlinePackage {
    ...
    pub fn discovered_backend(
        ...
        let provenance =
            ManifestProvenance::new(source_dir.join("pixi.toml"), ManifestKind::Pixi);
