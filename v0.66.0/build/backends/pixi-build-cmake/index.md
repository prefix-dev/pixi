...
1. **Configuration**: Runs `cmake` with the following default options:
...
These variants are used when you specify compilers in your [`[package.build.config.compilers]`](#compilers) configuration. Only `cxx_compiler` will be installed by default, the `c_compiler` is set to help when you would add that compiler.
